...
//                                                       int maxIterations, TelemetryPacket telemetry) {
//
...
//                reason = "converged";
//                telemetry.addLine("Converged after " + iteration + " iterations");
//                break;
//            }
//
...
//                        highAngle = midAngle;
//                        telemetry.addLine("Iteration %d: Angle %.1f° - No velocity found (angle too HIGH - can't reach horizontally) "+
//                                iteration+" "+ Math.toDegrees(midAngle));
//                    } else {
...
//                            highAngle = midAngle;
//                            telemetry.addLine("Iteration %d: Angle %.1f° - No velocity found (angle too HIGH - vertex before target)",
//                                    iteration, Math.toDegrees(midAngle));
//                        } else {
//                            // Vertex at/after target - on ascending part of parabola
//                            // Undershooting means angle is too low (not enough height)
//                            lowAngle = midAngle;
//                            telemetry.addLine("Iteration %d: Angle %.1f° - No velocity found (angle too LOW - falls short vertically)",
//                                    iteration, Math.toDegrees(midAngle));
//                        }
//                    }
//                }
...
//                        ));
//                        telemetry.addLine("Iteration %d: VALID - Angle %.1f°, Impact %.1f°, Distance %.1f°",
//                                iteration, Math.toDegrees(midAngle), Math.toDegrees(impactAngle),
//                                Math.toDegrees(distanceToMedian));
//                    } else {
//                        telemetry.addLine("Iteration %d: Angle %.1f°, Impact %.1f°, Distance %.1f° - Valid but not optimal(%.1f°)",
//                                iteration, Math.toDegrees(midAngle), Math.toDegrees(impactAngle),
//                                Math.toDegrees(distanceToMedian), Math.toDegrees(bestDistanceToMedian));
//                    }
//
...
//                        // Impact too steep, launch angle too high
//                        highAngle = midAngle;
//                        telemetry.addLine("Iteration %d: Impact angle %.1f° too STEEP",
//                                iteration, Math.toDegrees(impactAngle));
//                    } else {
//                        // Impact too shallow, launch angle too low
//                        lowAngle = midAngle;
//                        telemetry.addLine("Iteration %d: Impact angle %.1f° too SHALLOW",
//                                iteration, Math.toDegrees(impactAngle));
//                    }
//                    // Calculate y position at target x using projectile motion equation
//                    // y = x*tan(θ) - (g*x²)/(2*v²*cos²(θ))
...
//            telemetry.addLine("Max iterations (" + maxIterations + ") reached");
//        }
//
//        // If we found a valid solution, return it
//        if (bestSolution != null && Boolean.parseBoolean(bestSolution.get("success"))) {
//            telemetry.addLine("SUCCESS! Final solution: %.2f° at %.1f in/s",
//                    Double.parseDouble(bestSolution.get("angle")), Double.parseDouble(bestSolution.get("velocity")));
//            return bestSolution;
//        }
//
...
//            ));
//            telemetry.addLine("No solution found");
//        } else {
//            telemetry.addLine("Best effort: %.2f° at %.1f in/s (not valid)",
//                    Double.parseDouble(bestSolution.get("angle")), Double.parseDouble(bestSolution.get("velocity")));
//        }
//
//        return bestSolution;
