...
@Autonomous
public class AutonRedPathV2Motif extends LinearOpMode {
    ...
    CRServo hood=null;
    Pose2d beginPose=new Pose2d(72-11.9, 9, Math.toRadians(90));
    ...
    int intakecnt=0;
    int scannedMotifIndex=0;
    public static int DEFAULT_MOTIF_INDEX = 0;
    public static int MOTIF_BURST_COUNT = 3;
    public static double MOTIF_SCAN_TIMEOUT_MS = 1200;
    public static double MOTIF_SPIN_TIMEOUT_MS = 2500;
    public static double MOTIF_BURST_TIMEOUT_MS = 7000;
    public static double MOTIF_SETTLE_MS = 100;
    enum SpindexerState {
        STOPPED,
        HOLDING,
        INTAKE
...
//                outtake.updateHoodAngle();
     ...
     */
    public Action spinToMotif() {
        return spinToMotif(DEFAULT_MOTIF_INDEX, MOTIF_SCAN_TIMEOUT_MS, MOTIF_SPIN_TIMEOUT_MS);
    }

    public Action spinToMotif(int fallbackMotifIndex) {
        return spinToMotif(fallbackMotifIndex, MOTIF_SCAN_TIMEOUT_MS, MOTIF_SPIN_TIMEOUT_MS);
    }

    public Action spinToMotif(int fallbackMotifIndex, double scanTimeoutMs, double spinTimeoutMs) {
        return new SpinToMotifAction(fallbackMotifIndex, scanTimeoutMs, spinTimeoutMs);
    }

    public Action spinToMotifBurst() {
        return new SpinToMotifBurstAction(MOTIF_BURST_COUNT, DEFAULT_MOTIF_INDEX, MOTIF_SCAN_TIMEOUT_MS, MOTIF_BURST_TIMEOUT_MS);
    }

    public Action spinToMotifBurst(int shotCount, int fallbackMotifIndex) {
        return new SpinToMotifBurstAction(shotCount, fallbackMotifIndex, MOTIF_SCAN_TIMEOUT_MS, MOTIF_BURST_TIMEOUT_MS);
    }

    private class SpinToMotifAction implements Action {
        private final int fallbackMotifIndex;
        private final double scanTimeoutMs;
        private final double spinTimeoutMs;
        private final ElapsedTime timer = new ElapsedTime();
        private boolean initialized = false;
        private boolean scanDone = false;
        private int motifIndex = DEFAULT_MOTIF_INDEX;

        private SpinToMotifAction(int fallbackMotifIndex, double scanTimeoutMs, double spinTimeoutMs) {
            this.fallbackMotifIndex = Math.max(0, Math.min(2, fallbackMotifIndex));
            this.scanTimeoutMs = scanTimeoutMs;
            this.spinTimeoutMs = spinTimeoutMs;
        }
        ...
        public boolean run(TelemetryPacket packet) {
            ...
            if (!scanDone) {
                if (outtake.apriltag != null) {
                    ...
                    if (motifCode >= 1 && motifCode <= 3) {
                        ...
                        timer.reset();
                        spindexer.initSpin();
                    } else if (timer.milliseconds() >= scanTimeoutMs) {
                        motifIndex = fallbackMotifIndex;
                        scannedMotifIndex = motifIndex;
                        scanDone = true;
                        timer.reset();
                        spindexer.initSpin();
            ...
            boolean aligned = spindexer.spinToMotifV2(motifIndex);
            if (aligned) {
                spindexer.holdSpindexerOuttake();
                telemetry.addData("Motif", "Aligned");
                telemetry.addData("Motif Index", motifIndex);
                return false;
            }
            if (timer.milliseconds() >= spinTimeoutMs) {
                spindexerServo.setPower(0);
                telemetry.addData("Motif", "Spin Timeout");
                telemetry.addData("Motif Index", motifIndex);
                return false;
            }
            if (spindexer.detectioncnt == 3) {
                spindexer.initSpin();
            }

            telemetry.addData("Motif", "Spinning");
            telemetry.addData("Motif Index", motifIndex);
            return true;
        }
    }

    private class SpinToMotifBurstAction implements Action {
        private final int shotCount;
        private final int fallbackMotifIndex;
        private final double scanTimeoutMs;
        private final double burstTimeoutMs;
        ...
        private SpinToMotifBurstAction(int shotCount, int fallbackMotifIndex, double scanTimeoutMs, double burstTimeoutMs) {
            this.shotCount = Math.max(1, shotCount);
            this.fallbackMotifIndex = Math.max(0, Math.min(2, fallbackMotifIndex));
            this.scanTimeoutMs = scanTimeoutMs;
            this.burstTimeoutMs = burstTimeoutMs;
        }
        ...
        public boolean run(TelemetryPacket packet) {
            ...
            if (!scanDone) {
                if (outtake.apriltag != null) {
                    ...
                    if (motifCode >= 1 && motifCode <= 3) {
                        ...
                        scanDone = true;
                        spindexer.initSpin();
                    } else if (scanTimer.milliseconds() >= scanTimeoutMs) {
                        motifIndex = fallbackMotifIndex;
                        scannedMotifIndex = motifIndex;
                        scanDone = true;
                        spindexer.initSpin();
                    } else {
                        telemetry.addData("Motif Burst", "Scanning");
                        return true;
                    }
                } else {
                    motifIndex = fallbackMotifIndex;
            ...
            if (shotsCompleted >= shotCount) {
                spindexerServo.setPower(0);
                return false;
            }

            if (burstTimer.milliseconds() >= burstTimeoutMs) {
                spindexerServo.setPower(0);
                telemetry.addData("Motif Burst", "Timeout");
                telemetry.addData("Shots Sorted", shotsCompleted);
                return false;
            }

            if (settling) {
                spindexer.holdSpindexerOuttake();
                if (settleTimer.milliseconds() < MOTIF_SETTLE_MS) {
                    telemetry.addData("Motif Burst", "Settling");
                    telemetry.addData("Shots Sorted", shotsCompleted);
                    return true;
                }
                settling = false;
            ...
            if (aligned) {
                shotsCompleted++;
                motifIndex = (motifIndex + 1) % 3;
                settling = true;
                settleTimer.reset();
                telemetry.addData("Motif Burst", "Aligned");
                telemetry.addData("Shots Sorted", shotsCompleted);
                return true;
            }
            ...
            telemetry.addData("Motif Burst", "Sorting");
            telemetry.addData("Shots Sorted", shotsCompleted);
            telemetry.addData("Target Motif Index", motifIndex);
            return true;
        }
    }
    ...
    public class rotateSpindexer implements Action{
        private final Action inner = spinToMotifBurst();
        @Override
        public boolean run(TelemetryPacket packet) {
